...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        CompilationUnit compilationUnit = StaticJavaParser.parse(new File("C:\\github_fork\\autorest.java\\tests\\src\\test\\java\\fixtures\\bodydatetimerfc1123\\DateTimeRfc1123OperationsTests.java"));
        ...
        compilationUnit.getClassByName("DateTimeRfc1123OperationsTests").get().getMethods().stream()
                ...
                .forEach(method -> method.getBody().get().getStatements().stream().forEach(stmt -> {
                    stmt.accept(new GenericVisitorAdapter<Node, Void>() {
                        ...
                        public Node visit(MethodCallExpr n, Void arg) {
                            if (n.getName().getIdentifier().startsWith("get") && !n.getName().getIdentifier().endsWith("Async") && n.getScope().get().toString().startsWith("client")) {
                            ...
                            if (n.getName().getIdentifier().startsWith("put") && !n.getName().getIdentifier().endsWith("Async") && n.getScope().get().toString().startsWith("client")) {
        ...
        writeToFile(new File("C:\\github_fork\\autorest.java\\tests\\src\\test\\java\\fixtures\\bodydatetimerfc1123\\DateTimeRfc1123Operations1Tests.java"), code);
    }
